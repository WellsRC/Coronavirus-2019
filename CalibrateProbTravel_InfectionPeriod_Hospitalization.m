...
TAOT=TAO;
TAO(TAOT>=INDXMV)=TT(TAOT>=INDXMV)+TimeHospitalJan1(length(TAOT(TAOT>=INDXMV))); 
TAO(TAOT<INDXMV)=TT(TAOT<INDXMV)+TimeHospitalDec31(length(TAOT(TAOT<INDXMV))); 
TBNS=TAO;
...
pc=0.01.*[0.6:0.01:2];%linspace(1.3,3.5,101);
F=zeros(length(pc),1);

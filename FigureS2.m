...
XTL=datestr([startDateofSim+[0:(length(IncW(:,2))-1)]],'mm-dd-yy');
figure('units','normalized','outerposition',[0 0 1 1]);

subplot('Position',[0.065651260504202,0.64,0.921743697478991,0.3]); 
b=bar([1:length(IncW(:,2))],[ IncW(:,2) IncH(:,2) IncC(:,2) IncO(:,2)],'stacked','LineStyle','none'); hold on;
for ii=1:4
        b(ii).FaceColor = 'flat';
        b(ii).CData = CC(ii,:);
end
...
set(gca,'LineWidth',2,'tickdir','out','Fontsize',14,'XTick',[1:length(IncW(:,2))],'XTickLabel',XTL,'Xminortick','off','Yminortick','on');
xtickangle(45);
xlim([1 length(IncW(:,2))+0.5])
ylim([0 5000]);
yh=ylabel({'Incidence'},'Fontsize',18);
xlabel({'Date of symptom onset'},'Fontsize',18);
%Plot travel bans
plot([INDX INDX],[0 5000],'-.','color',[0.7 0.7 0.7],'LineWidth',1.5);
plot([INDX2 INDX2],[0 5000],'-.','color',[0.7 0.7 0.7],'LineWidth',1.5);
...
b=bar([1:length(IncW(:,2))],[ RWuhan(:,2) RHubei(:,2) RChina(:,2) OWHORep(:,2)],'stacked','LineStyle','none'); hold on;
for ii=1:4
        b(ii).FaceColor = 'flat';
        b(ii).CData = CC(ii,:);
end
...
% Lockdown Hubei
INDX2=datenum('01-25-2020')-datenum('12-06-2019')+1; % Need to add one since the week index for Dec 6 would be zero
set(gca,'LineWidth',2,'tickdir','out','Fontsize',14,'XTick',[1:length(IncW(:,2))],'XTickLabel',XTL,'Xminortick','off','Yminortick','on','YTick',[0:1000:14500]);
xtickangle(45);
xlim([1 length(IncW(:,2))+0.5])
ylim([0 14500]);
...
%Plot travel bans
plot([INDX INDX],[0 14500],'-.','color',[0.7 0.7 0.7],'LineWidth',1.5);
plot([INDX2 INDX2],[0 14500],'-.','color',[0.7 0.7 0.7],'LineWidth',1.5);
legend(b,{'Wuhan','Hubei (Outside Wuhan)','China (Outside Wuhan and Hubei)','International (Known Travel to China)'},'Location','NorthWest')
legend boxoff;

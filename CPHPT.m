...
TAO(TAOT>=INDXMV)=TT(TAOT>=INDXMV)+TimeHospitalJan1(length(TAOT(TAOT>=INDXMV)));  % for symptom onset on jan1 or past sample from that distribtuion (Note: here it is not an issue we truncated TT indexes before as we truncate this later as well and we are adding on to of the time of the travle ban)
TAO(TAOT<INDXMV)=TT(TAOT<INDXMV)+TimeHospitalDec31(length(TAOT(TAOT<INDXMV))); % for symptom onset before jan1 or past sample from that distribtuion (Note: here it is not an issue we truncated TT indexes before as we truncate this later as well and we are adding on to of the time of the travle ban)

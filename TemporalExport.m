...
TT=[repmat([T],NS2,1)-1 min(repmat([TW],NS2,1)-1,INDX-1) min(repmat([TF],NS2,1)-1,INDX2-1)];
D=(TT-E)+1;
D(D<0)=0;
...
E=repmat([T TW TF],NS2,1)-IP; 
TT=[repmat([T],NS2,1)-1 repmat([TW],NS2,1)-1 repmat([TF],NS2,1)-1];
D=(TT-E)+1;
D(D<0)=0;
...
TAO(TAO>=INDXMV)=TimeMedJan1(length(TAO(TAO>=INDXMV))); 
TAO(TAO<INDXMV)=TimeMedDec31(length(TAO(TAO<INDXMV))); 
D=(TT-E)+1;
D(D<0)=0;
